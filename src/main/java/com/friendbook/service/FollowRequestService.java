...
public class FollowRequestService {
    ...
    public void sendRequest(User sender, User receiver) {
        ...
        if (existing != null) {
            if (existing.getStatus() == FollowRequest.Status.ACCEPTED || existing.getStatus() == FollowRequest.Status.PENDING)
                return;
    ...
    public void followBack(User sender, User receiver) {
        ...
        FollowRequest existing = repo.findBySenderAndReceiver(sender, receiver);
        if (existing != null && existing.getStatus() == FollowRequest.Status.ACCEPTED) return;
        if (existing == null) {
    ...
    public boolean rejectRequest(Long requestId, User actingUser) {
        ...
        if (opt.isPresent()) {
            FollowRequest request = opt.get();
            if (!request.getReceiver().getId().equals(actingUser.getId())) return false;
